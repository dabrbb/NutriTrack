    ...
    const handleSaveProfile = async (e) => {
        ...
        try {
            ...
            if (error.response?.data?.errors) {
                ...
                setErrors({ general: [error.response?.data?.message || "Error al actualizar el perfil"] });
            }
        } finally {
    ...
    const handleUpdatePassword = async (e) => {
        ...
        try {
            ...
            if (error.response?.data?.errors) {
                ...
                setPasswordErrors({ current_password: [error.response.data.message] });
            }
        } finally {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4 overflow-y-auto">
            <div className="bg-white rounded-3xl p-8 max-w-md w-full shadow-2xl relative border border-gray-100 my-8 animate-in fade-in zoom-in-95 duration-200">
                ...
                <button onClick={handleClose} type="button" className="absolute top-6 right-6 text-gray-400 hover:text-gray-600 cursor-pointer">
                ...
                <form onSubmit={handleSaveProfile} className="space-y-4 border-b border-gray-100 pb-6">
                    ...
                    <Input
                        ...
                        error={profileErrors.birthday?.[0]}
                    />
                    ...
                    <div className="flex gap-4 pt-2">
                        <Button
                            type="button"
                            variant="ghost"
                            className="w-1/2 py-3 mt-0 text-gray-600 bg-gray-100 hover:bg-gray-200 border-none rounded-2xl text-sm font-semibold"
                            onClick={handleClose}>
                            Cancelar
                        </Button>
                        <Button
                            type="submit"
                            className="w-1/2 py-3 mt-0 rounded-2xl text-sm font-semibold shadow-none"
                ...
                <form onSubmit={handleUpdatePassword} className="space-y-4 pt-5">
                    ...
                    <Button
                        type="submit"
                        variant="ghost"
                        className="w-full py-3 mt-2 border border-gray-200 text-gray-700 hover:bg-gray-50 rounded-2xl text-sm font-semibold bg-transparent"
